...
import { demoResourceIds, SimulatedStatusProvider } from "../src/simulated-provider.js";
...
describe("simulated provider progression", () => {
  it("seeds every useful state and advances a stable resource deterministically", () => {
    ...
    const demo = new DemoController(store, new SimulatedStatusProvider(), clock);

    const seeded = demo.reset();
    expect(seeded.version).toBe(1);
    expect(seeded.providers).toMatchObject([{ id: "demo", connectivity: "connected" }]);
    ...
    const expectedStatuses = ["waiting", "running", "completed", "failed", "running"] as const;

    for (const [index, status] of expectedStatuses.entries()) {
      now = new Date(now.getTime() + 1_000);
      const result = demo.advance();
      const live = result.snapshot.resources.find(({ id }) => id === demoResourceIds.live);

      expect(live?.status).toBe(status);
      expect(result.snapshot.resources.map(({ id }) => id).sort()).toEqual(stableIds);
      expect(result.snapshot.version).toBe(index + 2);
      expect(
        result.snapshot.events.some(({ resourceId }) => resourceId === demoResourceIds.live),
      ).toBe(true);
    }
  });
  ...
  it("resets progression while preserving resource identifiers", () => {
    ...
    expect(reset.resources.find(({ id }) => id === demoResourceIds.live)?.status).toBe("running");
